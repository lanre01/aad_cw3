...
-- | Ukkonen suffix tree implementation over strict 'ByteString' values.
--
-- Construction uses a pre-allocated mutable boxed vector
-- for O(1) worst-case node operations within the 'ST' monad.
-- A suffix tree for an input of length n contains at most
-- 2n - 1 tree nodes; adding the bottom, root and end character sentinels gives
-- a maximum of 2n + 4 (2(n + 1) + 2) entries, so the vector is sized at @2 * n + 4@ up front 
-- and never resized.
--
...
-- | Convert a symbol to the 'IntMap' key used for outgoing transitions.
-- Byte values keep their numeric value, while the unique end sentinel is
-- placed just after the byte range.
symbolKey :: Symbol -> Int
symbolKey (Byte w) = fromIntegral w
symbolKey End      = 256

-- | Read the symbol at a text position, treating the position immediately
-- after the last byte as the explicit end sentinel.
...
-- | Strict variant of tuples and triples to prevent
-- heap alloation in hot loops
...
-- | Specialised Maybe for int to allow unpacking
data StrictIntMaybe
  = SNothing
...
--
-- The vector is pre-sized to @2 * n + 4@ where @n@ is the input length.
-- A suffix tree contains at most @2n - 1@ tree nodes,
-- plus the bottom, root and end character sentinels, giving @2n + 4@ entries.
...
-- | Set or replace the suffix link for a node.
...
-- | Update the suffix tree with the current index.
